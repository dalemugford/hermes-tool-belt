...
> **2026-05-31 NOTE — kept for the cost-categorization table.** This doc
> describes the per-turn predictor path, which is now the cache-off
> fallback (kimi, gpt-5.4-mini). The cache-on default is session-boundary
> freezing per [16.cache-aware-refactor-plan-2026-05-30.md](16.cache-aware-refactor-plan-2026-05-30.md).
> The categorization table below (always-on candidates, trigger groups,
> per-tool token costs) is still the canonical rationale for `policy.yaml`'s
> structure and applies under both modes — the table informs the
> session-start freeze ceiling on cache-on, and per-turn triggers on
> cache-off.
